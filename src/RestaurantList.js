...
class RestaurantList extends Component {
	constructor(props) {
		...
		this.state = { lat: 0, long: 0, value: '', restaurants: [], showLoader: false };
	...
	async restaurants() {
		try {
			...
			let req = await axios.get('https://developers.zomato.com/api/v2.1/search', {
				headers: {
			...
			this.setState({ showLoader: false, restaurants: [ ...this.state.restaurants, ...rest ] });
		} catch (err) {
			console.log(err);
		}
	}
